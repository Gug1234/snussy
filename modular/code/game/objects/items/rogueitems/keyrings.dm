...
/obj/item/storage/keyring/proc/get_chastity_key_for_target(mob/living/carbon/human/target)
	if(!target)
		return null

	// Prefer the exact matching key if the target is wearing a lockable chastity device.
	if(target.chastity_device && target.chastity_device.lockable)
		for(var/obj/item/roguekey/chastity/key in contents)
			if(key.lockhash == target.chastity_device.lockhash)
				return key

	// Fallback to any chastity key so existing warning/mismatch messaging still applies.
	for(var/obj/item/roguekey/chastity/key in contents)
		return key

	return null

/obj/item/storage/keyring/attack_self(mob/user)
	if(ishuman(user))
		var/mob/living/carbon/human/target = user
		var/obj/item/roguekey/chastity/key = get_chastity_key_for_target(target)
		if(key)
			return key.attack(target, user, user.zone_selected)
	return ..()

/obj/item/storage/keyring/attack(mob/M, mob/user, def_zone)
	if(ishuman(M))
		var/mob/living/carbon/human/target = M
		var/obj/item/roguekey/chastity/key = get_chastity_key_for_target(target)
		if(key)
			return key.attack(target, user, def_zone)
	return ..()
