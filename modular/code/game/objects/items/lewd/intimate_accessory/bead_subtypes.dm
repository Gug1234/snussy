...
/obj/item/intimate_accessory/rear/plug/analbeads/glass/on_ripcord(mob/user, mob/living/carbon/human/target, violent = FALSE)
	...
	if(violent)
		// Violent ripcord shatters a bead inside, same consequence as falling from a z-level
...
/// Gate behind extreme ERP, shattering glass inside someone is obviously no beuno for a lot of people
...
/obj/item/intimate_accessory/rear/plug/analbeads/glass/proc/shatter_on_z_impact(mob/living/carbon/human/victim)
	...
	// fuck up the groin of our bead bozo
	var/obj/item/bodypart/groin = victim.get_bodypart(check_zone(BODY_ZONE_PRECISE_GROIN))
	if(groin)
		groin.add_wound(/datum/wound/slash/large)//tempted to make this an artery but this is fine for now. maybe 
		groin.add_wound(/datum/wound/slash/large)

	// remove and destroy the beads, it's part of their colon now
	beads_inserted = 0
...
// MIXED MEDIUM+LARGE, Alternating medium and large, 8 total. Gem-socketable.
...
// SPIKED, 6 spiked morningstar balls. Unsocketable. Extreme ERP only for obvious reasons
...
// GIANT, 6 fist-sized balls. Tauric sized but psydon's strongest tabaxi confessors are surely up to the challenge. Gem-socketable.
...
// Mental note to move chastity to this, maybe.
...
/obj/item/unfinished_analbeads/proc/customize(mob/living/user)
	...
	var/is_spiked = istype(new_beads, /obj/item/intimate_accessory/rear/plug/analbeads/spiked)//no metal variations, spiked is spiked
	if(!is_spiked)
