...
/obj/item/intimate_accessory/piercing/tongue/zizite/proc/speak_in_zizo_chant(mob/living/carbon/human/user)
	...
	user.say(chant_message, language = /datum/language/undead)//lets them speak in z chant but not understand a word of it. Funny!
	return TRUE
...
/mob/living/carbon/human/proc/speak_zizo_chant()
	...
	if(!piercing)
		to_chat(src, span_warning("I need to be wearing a zizite tongue piercing."))//ideally, we wont ever ever see this fallback text
		return FALSE
...
/mob/living/carbon/human/proc/quote_psydonic_scripture()
	...
	if(!piercing)
		to_chat(src, span_warning("I need to be wearing a psydonic tongue piercing."))//same here
		return FALSE
