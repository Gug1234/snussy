...
/// Round-start chastity device preference for the active character slot.
/datum/preferences/var/pref_chastity_enabled = FALSE
/// Prefer flat front cages where the selected anatomy supports one.
/datum/preferences/var/pref_chastity_flat = FALSE
/// Add an anal shield where the selected chastity device supports one.
/datum/preferences/var/pref_chastity_anal = FALSE
/// Prefer spiked variants. This is still gated by extreme ERP.
/datum/preferences/var/pref_chastity_spiked = FALSE
/// Spawn the selected chastity device locked.
/datum/preferences/var/pref_chastity_locked = FALSE
/// Generate the device key for the wearer at round start.
/datum/preferences/var/pref_chastity_spawn_key = TRUE
/// Optional character-name key recipients for round-start copies.
/datum/preferences/var/list/pref_chastity_key_stashes = null
/// Allow one random opted-in player to receive a key copy.
/datum/preferences/var/pref_chastity_random_keys = FALSE
 ...
 *   - Key generation (if pref_chastity_spawn_key is TRUE and this is not a preview dummy)
...
/datum/preferences/proc/apply_chastity_preferences(mob/living/carbon/human/H)
	...
	if(!chastenable || !pref_chastity_enabled)
		return
	var/visual_only = istype(H, /mob/living/carbon/human/dummy)
	...
	// Key handling
	if(!visual_only && pref_chastity_spawn_key)
	...
	if(!visual_only && pref_chastity_key_stashes && length(pref_chastity_key_stashes) && device.generated_key)
	...
	if(!visual_only && pref_chastity_random_keys && device.generated_key)
