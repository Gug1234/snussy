...
/datum/sex_action/tonguebath/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	var/internal_retained = target.sexcon?.target_retains_internal_creampie(target)
	if(user.zone_selected == BODY_ZONE_PRECISE_GROIN && creampie && !internal_retained)
	...
	if(user.zone_selected == BODY_ZONE_PRECISE_GROIN && target.sexcon.has_chastity_cage())
		user.visible_message(user.sexcon.spanify_force("[user] licks around the gaps of [target]'s chastity device..."))
		playsound(user, pick('sound/misc/mat/mouthend (1).ogg','sound/misc/mat/mouthend (2).ogg'), 100, FALSE, ignore_walls = FALSE)
	if(user.zone_selected == BODY_ZONE_PRECISE_GROIN && target.sexcon.has_chastity_cage() && creampie && !internal_retained)
		user.visible_message(user.sexcon.spanify_force("[user] licks up the cum dripping from the gaps in [target]'s chastity device..."))
		playsound(user, pick('sound/misc/mat/mouthend (1).ogg','sound/misc/mat/mouthend (2).ogg'), 100, FALSE, ignore_walls = FALSE)
		creampie.clean_up(null, CLEAN_WEAK)
