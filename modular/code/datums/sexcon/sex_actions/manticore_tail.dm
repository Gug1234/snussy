...
/datum/sex_action/manticore_pear_anal/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user]'s tail presses its sealed bud against [target]'s rim, the interlocking plates grinding together as it forces its way inside with a cruel, deliberate slowness."))
...
/datum/sex_action/manticore_pear_anal/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user] rips [user.p_their()] tail free from [target]'s destroyed rear, the plates snapping shut with a wet crunch, leaving behind a gaping, prolapsed ruin."))
...
/datum/sex_action/manticore_pear_oral/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user]'s tail forces its sealed bud past [target]'s lips, the ridged plates scraping across teeth and gums as it pushes deep into [target]'s mouth."))
	playsound(target, 'sound/misc/mat/insert (1).ogg', 35, TRUE, ignore_walls = FALSE)

/datum/sex_action/manticore_pear_oral/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("The bud inside [target]'s mouth begins to blossom, the bonelike plates cranking apart with agonizing slowness, forcing [target]'s jaw wider and wider until the joints pop and creak, teeth cracking against the unyielding chitin."))
	playsound(target, 'sound/combat/fracture/fracturewet (1).ogg', 40, TRUE, ignore_walls = FALSE)
	user.sexcon.perform_sex_action(target, 0, 10, TRUE)

/datum/sex_action/manticore_pear_oral/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user] wrenches [user.p_their()] tail free from [target]'s ruined mouth, the plates folding shut as they drag loose teeth and blood with them, leaving [target]'s jaw hanging at a sickening angle."))
	// Apply jaw fracture
...
/datum/sex_action/manticore_pear_vaginal/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user]'s tail pushes its sealed bud between [target]'s legs, the interlocking plates pressing against [target]'s entrance before forcing inside with a sick, grinding push."))
	playsound(target, 'sound/misc/mat/insert (1).ogg', 35, TRUE, ignore_walls = FALSE)

/datum/sex_action/manticore_pear_vaginal/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("The tail bud blossoms inside [target], the plates spreading apart with merciless force, stretching walls that were never meant to give this far. Something tears deep inside, the wet sound of ripping flesh drowned out by [target]'s agonized screaming."))
	playsound(target, 'sound/combat/fracture/fracturewet (1).ogg', 40, TRUE, ignore_walls = FALSE)
	user.sexcon.perform_sex_action(target, 0, 10, TRUE)

/datum/sex_action/manticore_pear_vaginal/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_userdanger("[user] tears [user.p_their()] tail free from [target]'s savaged cunt, the plates folding shut with a crunch, dragging blood and tissue with them."))
