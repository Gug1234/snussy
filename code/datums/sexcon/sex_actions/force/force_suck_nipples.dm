...
/datum/sex_action/force_suck_nipples/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_front_piercing(user))
		user.visible_message(span_warning("[user] forces [target]'s head down to swallow and suck on [user.p_their()] pierced nipples!"))
	else
...
/datum/sex_action/force_suck_nipples/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_front_piercing(user))
		user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] forces [target] to suck and pull on [user.p_their()] pierced nipples."))
	else
	...
	apply_silver_intimate_contact("breast", user, target)
