	...
	if(splashed_user && (oral || !splashed_user.sexcon.knotted_status))
		if(oral)
			...
			if(!facial)
				...
				facial.refresh_cum()
				splashed_user.visible_message(span_love("[splashed_user] takes another load in their mouth!"), span_love("I take another load in my mouth!"))
			if(splashed_user.reagents)
